...
import { upload } from "../../lib/upload.js"; // shared upload
import { doc, updateDoc } from "firebase/firestore";
import { resolveAvatar, sanitizeImageUrl } from "../../lib/media.js";
...
const Settings = ({
  ...
  const ProfileSection = () => {
    ...
    const [avatarURL, setAvatarURL] = useState(sanitizeImageUrl(user?.avatar));
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        let finalUrl = sanitizeImageUrl(avatarURL);

        if (avatarFile) {
          finalUrl = await upload(avatarFile, "avatars", {
        ...
        toast.error(err?.message || "Impossible d'enregistrer le profil");
      } finally {
        setIsSaving(false);
      }
    };
        ...
        <form className="profile-form" onSubmit={handleSubmit}>
          ...
          >
            ...
            <div className="telegram-status-toggle">
              ...
              <div
                className={`status-slider ${status === "Privé" ? "right" : "left"}`}
              />
            </div>
          </div>
          <div className="profile-row">
          ...
          {showQR && (
            <div
              style={{
                ...
                marginBottom: "16px",
                animation:
                  "qrPopIn 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards",
              }}
            >
              <div
                style={{
  ...
  const ThemeSection = () => {
    ...
    const [selectedBg, setSelectedBg] = useState(
      () => localStorage.getItem("chat_background") || "#e5ddd5",
    );
    ...
    const handleBgChange = (bg) => {
      ...
      window.dispatchEvent(
        new CustomEvent("chatBackgroundChanged", { detail: bg }),
      );
    };
    ...
    return (
      <div className="settings-section">
        ...
        <div className="settings-list" style={{ padding: "0 14px" }}>
          ...
          <div
            className="telegram-status-toggle"
            ...
            {defaultBackgrounds.map((bg, idx) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  background:
                    bg.startsWith("http") || bg.startsWith("data:")
                      ? `url(${bg}) center/cover`
                      : bg,
                  border:
                    selectedBg === bg
    ...
    <>
      ...
      <div className="settings-profile">
        <div className="profile-avatar">
          {sanitizeImageUrl(user?.avatar) ? (
            <img src={user.avatar} alt="avatar" />
          ) : (
            (user?.name || user?.username || "UT").slice(0, 2).toUpperCase()
          )}
        </div>
      ...
      <div className="settings-list">
        <div className="settings-item" onClick={toggleTheme}>
          <div
            ...
            style={{ backgroundColor: "#5856d6", color: "#fff" }}
          >
        ...
        >
          <div
            className="settings-icon"
            style={{ backgroundColor: "#32ade6", color: "#fff" }}
          >
            <Palette size={18} />
          </div>
          <span>Apparence</span>
